...
async function parseTargetFolder(rawValue: string) {
...
export const rbvpNamespaceProvider: RBVPNamespaceProvider = {
  ...
  validateAction: rawValue => {
    parseTargetFolder(rawValue)
  },
  resolveAction: async rawValue => {
    const targetFolder = await parseTargetFolder(rawValue)
    return {
      namespace: componentName,
      displayValue: targetFolder.title,
  ...
  execute: async (action: RBVPResolvedAction, context: RBVPEngineContext) => {
    ...
    try {
      getOption().targetFolderID = action.value as number
